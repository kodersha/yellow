<?php
// MicroFeed extension, https://github.com/annaesvensson/yellow-microFeed

class YellowMicroFeed {
    ...
    public function onLoad($yellow) {
        $this->yellow = $yellow;
        $this->yellow->system->setDefault("microFeedLocation", "/microfeed/");
        $this->yellow->system->setDefault("microFeedFileXml", "microfeed.xml");
        $this->yellow->system->setDefault("microFeedFilterLayout", "none");
        $this->yellow->system->setDefault("microFeedPaginationLimit", "30");
    }
    ...
    public function onParsePageLayout($page, $name) {
        if ($name=="micro-feed") {
            ...
            $microFeedFilterLayout = $this->yellow->system->get("microFeedFilterLayout");
            if ($microFeedFilterLayout!="none") $pages->filter("layout", $microFeedFilterLayout);
            $chronologicalOrder = ($this->yellow->system->get("microFeedFilterLayout")!="blog");
            if ($this->isRequestXml($page)) {
                $pages->sort($chronologicalOrder ? "modified" : "published", false);
                $entriesMax = $this->yellow->system->get("microFeedPaginationLimit");
                ...
                foreach ($pages as $pageMicroFeed) {
                    $timestamp = strtotime($pageMicroFeed->get($chronologicalOrder ? "modified" : "published"));
                    $content = $this->yellow->toolbox->createTextDescription($pageMicroFeed->getContent(), 0, false, "<!--more-->", "<a href=\"".$pageMicroFeed->getUrl()."\">".$this->yellow->language->getTextHtml("blogMore")."</a>");
                    $output .= "<item>\r\n";
                    $output .= "<link>".$pageMicroFeed->getUrl()."</link>\r\n";
                    $output .= "<pubDate>".date(DATE_RSS, $timestamp)."</pubDate>\r\n";
                    $output .= "<guid isPermaLink=\"false\">".$pageMicroFeed->getUrl()."?".$timestamp."</guid>\r\n";
                    $output .= "<dc:creator>".$pageMicroFeed->getHtml("author")."</dc:creator>\r\n";
                    $output .= "<description>".$pageMicroFeed->getHtml("description")."</description>\r\n";
                ...
                $this->yellow->page->set("microFeedChronologicalOrder", $chronologicalOrder);
                $this->yellow->page->setPages("microFeed", $pages);
                $this->yellow->page->setLastModified($pages->getModified());
            }
        }
    }
    ...
    public function isRequestXml($page) {
        return $page->getRequest("page")==$this->yellow->system->get("microFeedFileXml");
    }
}
